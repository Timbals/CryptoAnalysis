				...
				switch(settings.getRulesetPathType()) {
					case DIR:
						try {
							...
							rules.addAll(CrySLRuleReader.readFromDirectory(ruleSetDir));
					...
					case ZIP:
						try {
							File ruleSetZip = new File(settings.getRulesetPathZip());
							rules.addAll(CrySLRuleReader.readFromZipFile(ruleSetZip));
